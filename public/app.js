...
function activeTenantForFlat(flatId){
  return state.tenants.find(t=>t.flat_id===flatId && (t.status||"active")==="active") || null;
}
function latestReadingForFlat(flatId){
  const rows=state.bills.filter(b=>b.flat_id===flatId && b.current_reading!==null && b.current_reading!==undefined);
  rows.sort((a,b)=>String(b.month||"").localeCompare(String(a.month||"")));
  return rows[0]?.current_reading ?? "";
}
function flatBillInfo(flatId){
  const f=state.flats.find(x=>x.id===flatId);
  const t=activeTenantForFlat(flatId);
  const bills=state.bills.filter(b=>b.flat_id===flatId).sort((a,b)=>String(b.month||"").localeCompare(String(a.month||"")));
  const latest=bills[0];
  return {flat:f,tenant:t,latest};
}
function openPayment(){
 openModal("Record Payment",`
 <select class="input" name="flat_id" id="payFlat" required><option value="">Select flat</option>${flatOptions()}</select>
 <div id="payTenantBox" class="rounded-xl bg-slate-50 border p-3 text-sm">Select a flat to fetch its active tenant.</div>
 <input type="hidden" name="tenant_id" id="payTenant">
 <select class="input" name="bill_id" id="payBill"><option value="">Select bill</option></select>
 <div id="payBillInfo" class="text-sm text-slate-500"></div>
 ...
 <button class="btn primary w-full">Record Payment</button>`,async fd=>{
  try{
   ...
   if(!body.tenant_id) body.tenant_id=null;
   await api("/api/payments",{method:"POST",body:JSON.stringify(body)});
   closeModal();toast("Payment recorded and due recalculated");await load()
  }catch(e){toast(e.message,false)}
 });
 const flatEl=$("payFlat");
 flatEl.addEventListener("change",()=>{
   const id=flatEl.value, info=flatBillInfo(id), t=$("payTenant"), box=$("payTenantBox"), bills=state.bills.filter(b=>b.flat_id===id);
   if(info.tenant){t.value=info.tenant.id;box.innerHTML=`<b>Tenant:</b> ${esc(info.tenant.name)}<br><span class="text-slate-500">${esc(info.tenant.mobile||"")}</span>`}
   else{t.value="";box.innerHTML=`<span class="text-amber-700">No active tenant for this flat.</span>`}
   $("payBill").innerHTML=`<option value="">Select bill</option>${bills.map(b=>`<option value="${b.id}">${esc(b.month)} — Total ${money(b.total_amount)} — Due ${money(b.total_due)}</option>`).join("")}`;
 });
 $("payBill").addEventListener("change",()=>{
   const b=state.bills.find(x=>x.id===$("payBill").value);
   $("payBillInfo").textContent=b?`Bill total ${money(b.total_amount)} · Current due ${money(b.total_due)}`:"";
 });
}
function openBill(){
 openModal("Generate Monthly Bill",`
 <select class="input" name="flat_id" id="billFlat" required><option value="">Select flat</option>${flatOptions()}</select>
 <div id="billTenantBox" class="rounded-xl bg-slate-50 border p-3 text-sm">Select a flat to fetch its active tenant.</div>
 <input type="hidden" name="tenant_id" id="billTenant">
 <input class="input" name="month" type="month" value="${month()}" required>
 <div class="grid grid-cols-2 gap-3">
   <div><label class="text-sm text-slate-500">Previous Reading</label><input class="input mt-1 bg-slate-50" id="previousReading" readonly></div>
   <div><label class="text-sm text-slate-500">Current Reading</label><input class="input mt-1" name="current_reading" id="currentReading" type="number" min="0" step="0.01" required></div>
 </div>
 <div id="billCalc" class="rounded-xl bg-indigo-50 border border-indigo-100 p-4 text-sm">Select a flat to calculate.</div>
 <button class="btn primary w-full">Generate Bill</button>`,async fd=>{
  try{
   const body=Object.fromEntries(fd);
   const prev=Number($("previousReading").value||0),cur=Number(body.current_reading||0);
   if(cur<prev) throw Error(`Current reading cannot be lower than previous reading (${prev}).`);
   await api("/api/bills",{method:"POST",body:JSON.stringify(body)});
   closeModal();toast("Bill generated");await load()
  }catch(e){toast(e.message,false)}
 });
 const flatEl=$("billFlat");
 const refresh=()=>{
   const id=flatEl.value, info=flatBillInfo(id), prev=latestReadingForFlat(id);
   $("previousReading").value=prev;
   if(info.tenant){$("billTenant").value=info.tenant.id;$("billTenantBox").innerHTML=`<b>Tenant:</b> ${esc(info.tenant.name)}<br><span class="text-slate-500">${esc(info.tenant.mobile||"")}</span>`}
   else{$("billTenant").value="";$("billTenantBox").innerHTML=`<span class="text-amber-700">No active tenant for this flat.</span>`}
   const rate=Number(info.flat?.electricity_rate||0),rent=Number(info.flat?.monthly_rent||0);
   $("currentReading").oninput=()=>{const cur=Number($("currentReading").value||0),units=Math.max(0,cur-Number(prev||0)),elec=units*rate,total=rent+elec;$("billCalc").innerHTML=`<div class="grid grid-cols-2 gap-2"><span>Monthly Rent</span><b>${money(rent)}</b><span>Units</span><b>${units}</b><span>Electricity (${money(rate)}/unit)</span><b>${money(elec)}</b><span class="border-t pt-2">Estimated Total</span><b class="border-t pt-2">${money(total)}</b></div>`}
   $("currentReading").dispatchEvent(new Event("input"));
 };
 flatEl.addEventListener("change",refresh);
}
